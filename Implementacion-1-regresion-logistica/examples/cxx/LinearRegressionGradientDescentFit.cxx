...
int main( int argc, char** argv )
{
  ...
  using TModel = PUJ_ML::Model::Regression::Linear< TReal, TNatural >;
  ...
  if( argc > 3 ) std::istringstream( argv[ 3 ] ) >> L1;
  if( argc > 4 ) std::istringstream( argv[ 4 ] ) >> L2;
  if( argc > 5 ) std::istringstream( argv[ 5 ] ) >> train_size;
  if( argc > 6 ) std::istringstream( argv[ 6 ] ) >> max_epochs;
  ...
  PUJ_ML::Optimizer::GradientDescent< TModel > opt( m );
  opt.setAlpha( alpha );
  opt.setLambda1( L1 );
  opt.setLambda2( L2 );
...
// eof - LinearRegressionGradientDescentFit.cxx
